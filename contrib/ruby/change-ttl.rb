...
# == License
#
#   Copyright (C) 2016 Hideki Okamoto
#
...
opt.on('-r RECORD_TYPE', 'Comma separated record types') { |v| params[:record_type] = v }
opt.on('-d CONSUMER_DOMAIN', '(Optional) Base URL will be https://[CONSUMER_DOMAIN].luna.akamaiapis.net') { |v| params[:consumer_domain] = v }
opt.on('-c CLIENT_TOKEN', '(Optional) Client Token') { |v| params[:client_token] = v }
opt.on('-s CLIENT_SECRET', '(Optional) Client Secret') { |v| params[:client_secret] = v }
opt.on('-a ACCESS_TOKEN', '(Optional) Access Token') { |v| params[:access_token] = v }
...
if [params[:zone], params[:ttl], params[:record_type]].any? { |v| v.nil? }
  puts opt.help
  exit
end

# Initialize Edgegrid object
if [params[:consumer_domain], params[:client_token], params[:client_secret], params[:access_token]].any? { |v| v.nil? }
...
affected_record_type = params[:record_type].split(',').map { |v| v.strip.upcase }.uniq

request = Net::HTTP::Get.new URI.join(baseuri.to_s, "/config-dns/v1/zones/#{params[:zone]}").to_s
response = http.request(request)

zone_info = JSON.load(response.body)
...
if params[:go]
  ...
  request.body = JSON.generate(zone_info)
