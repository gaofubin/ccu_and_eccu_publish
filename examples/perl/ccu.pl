...
my $url = URI->new("$baseurl/ccu/v2/queues/default");
my $response = $agent->get($url);
my $queue_object = decode_json($response->content);
if ($verbose == 1) {print $json->encode($queue_object); }
...
my $req = HTTP::Request->new(
	POST => "$baseurl/ccu/v2/queues/default");
$req->content_type('application/json');

my $content = '{ "objects" : [ "http://bc.akamaiapibootcamp.com/index.html" ] }';
$req->content($content);
